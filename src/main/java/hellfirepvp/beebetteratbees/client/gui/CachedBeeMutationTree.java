...
public class CachedBeeMutationTree extends CachedRecipe {
    ...
    public CachedBeeMutationTree(IBeeMutation parentMutation) {
        ...
        this.mutationTree = new SimpleBinaryTree<IAllele>(4, parentMutation.getTemplate()[0],
        ...
        PositionedMutationNodeStack leftChild = placeInRenderBuffer(mutationTree.getRoot().getLeftNode(),
                Y_OFFSET + yStep, yStep,
                MIN_X, center, iterationDepth - 1);
        PositionedMutationNodeStack rightChild = placeInRenderBuffer(mutationTree.getRoot().getRightNode(),
                Y_OFFSET + yStep, yStep,
                center, this.evaluatedMaxX, iterationDepth - 1);

        List<IBeeMutation> mutationsToRoot = getMutationsWithResult(mutationTree.getRoot().getValue());
        float ch = -1;
        ...
        this.rootStack = new PositionedMutationNodeStack(
                createStack((IAlleleSpecies) mutationTree.getRoot().getValue(), BEE_TYPE_PRINCESS),
                (MIN_X + this.evaluatedMaxX) / 2, Y_OFFSET,
                ch, requirements,
                leftChild, rightChild, true);
    }

    private void removeAndReplaceDuplicates(SimpleBinaryTree.Node<IAllele> node, List<IAllele> discoveredMutations) { //Replace with 2 lines.
    ...
    */
    ...
    public static class PositionedMutationNodeStack extends PositionedStack {
        ...
        public PositionedMutationNodeStack(Object object, int x, int y, boolean genPerms,
                                           float baseChance, Collection<String> requirementInfo,
                                           PositionedMutationNodeStack leftChild, PositionedMutationNodeStack rightChild,
        ...
        public PositionedMutationNodeStack(Object object, int x, int y,
                                           float baseChance, Collection<String> requirementInfo,
                                           PositionedMutationNodeStack leftChild, PositionedMutationNodeStack rightChild,
                                           boolean hasPossibleChildren) {
            super(object, x, y);
